...
def getInput(inp_file,out_file,outw_file):
    ...
    while(not(line=='\n')):
        ...
        reliability_score = (svd_med-svd_min)/(svd_max*math.exp(svd_max-svd_med))
        split0_weight = 2*reliability_score 
        split1_weight = reliability_score
        split2_weight = reliability_score
        print split0_weight,split1_weight,split2_weight
